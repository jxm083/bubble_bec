...
(* v2: remove those constants which are defined in the new package BatesConstants.m to prevent them 
from being duplicated here. *)
...
(* Some of the variables here are defined in terms of the variables in BatesConstants.m *)
\[Mu]m=10^-6;
Gauss=10^-4;
G= Gauss;
cm = .01;
mA=mm=10^-3;
A=1;
(*\[Mu]0=mu0;*)(*4\[Pi] 10^-7;*) (*[N/A^2]*)
(*h = 6.62606957 10^-34;
hbar = h /(2 \[Pi]);*)
(*\[HBar]=hbar;*)
kb = kB;(*1.3806488*^-23;*) (*[J/K]*)
mRb =mRb87;(*1.443160648*^-25;*) (* [kg] *)
(*\[Mu]B=muB;*)(*9.27400968*^-24;*) (* [J/T] *)
gfactor=1/2;
gF = 1/2; (* not accurate to .0001 *) 
gF= gJ (F(F+1)-Inuc(Inuc+1)+J(J+1))/(2F(F+1))+gI (F(F+1)+Inuc(Inuc+1)-J(J+1))/(2F(F+1)) /. {F->2,J->1/2};
Inuc= 3/2;
Ahfs =  ahfsS12/h;(*3.417341305*^9;*) (* [Hz] *)
(*gI= -.0009951414 ;*)
gJ = gJS12;(*2.00233113;*)
nKPerHz = 1*^9(h/kb);
...
Block[
{
k = ((gJ-gI)muB/Ahfs/h/2), (* aka (gJ-gI)muB/(2*Ahfs*h), so that Steck's x = h*k*B *)
\[CapitalXi],
Field (* [T] *)
},
...
ZeemanShift[Field_]= {
-Ahfs-2gI muB Field/h+\[CapitalXi][Field]Ahfs Sqrt[1- 2k Field+k^2 Field^2],
-Ahfs-gI muB Field/h+Ahfs Sqrt[1- k Field+k^2 Field^2],
-Ahfs+Ahfs Sqrt[1+k^2 Field^2],
-Ahfs+ gI muB Field/h+Ahfs Sqrt[1+k  Field+k^2 Field^2],
-Ahfs+ 2 gI muB Field/h+ Ahfs Sqrt[1+2k  Field+k^2 Field^2]
} ;(* F=2, m= -2, -1, 0, +1, +2 *)
...
(*Plot[{ZeemanShift[Field]/1*^6,0, gF(muB/h/1*^6) Field,-gF(muB/h/1*^6) Field},
{Field,0,5000 Gauss},
Frame\[Rule]True,
...
-Ahfs/4-2gI muB Field/h+\[CapitalXi][Field]Ahfs Sqrt[1- 2k Field+k^2 Field^2],
-Ahfs/4-gI muB Field/h+Ahfs Sqrt[1- k Field+k^2 Field^2],
-Ahfs/4+Ahfs Sqrt[1+k^2 Field^2],
-Ahfs/4+ gI muB Field/h+Ahfs Sqrt[1+k  Field+k^2 Field^2],
-Ahfs/4+ 2 gI muB Field/h+ Ahfs Sqrt[1+2k  Field+k^2 Field^2]
} ;(* F=2, m= -2, -1, 0, +1, +2 *)
...
Block[
{
x = (gJ-gI)*muB/(2*h*Ahfs), (* N.B. this term differs from Steck because our shift is given in [Hz], not [J] *)
field (* [T] *)
},
ZeemanShiftF1[field_] = {
-Ahfs/4 + gI*muB*(-1)*field/h - Ahfs*Sqrt[1 + (-1)*x*field + x^2*field^2],
...
Diagonal[Sqrt[muB Abs[DMatrixDiag]/mRb]/(2\[Pi]) ]}(* in Hz *)
]
];
