...
\[Delta]:=SingleTrapFrequency[trap1,trap2,newramp,nramps,position +1][[3]]-model[ramp[[position+1,1]]];
...
(*Print[{newramp\[LeftDoubleBracket]2,position\[RightDoubleBracket],\[Delta],10.^round}];*)
];
...
\[Omega]i=startpts[[1,3]];
\[Omega]f=startpts[[2,3]];
{\[Omega]i,\[Omega]f}
]
...
newramp=BinaryFreqSearch[trap1,trap2,\[Omega]i,\[Omega]f,ramp,points,position,fraction,round][[1]];
(*Print[position,newramp];*)
position++;
ramp=newramp
];
ramp
...
(* ::Input::Initialization:: *)
ExpansionPlotHacked[movingtrap_,npoints_,ramp_:{{},{}},OptionsPattern[{axis->"z",trap->"1",usemodel->False,frequency->True,trapposition->True,gridlines->False,size->Medium,meanfrequency->False,meanmodel->False}]]:=Block[{
