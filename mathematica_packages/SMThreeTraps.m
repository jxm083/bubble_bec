...
AZ2sel = 1,
AZ1 = -0.686,
AZ2 = -0.(*0001*), (* originally reported as -0.0001, now (1 July) it seems to be 0. *)
H1pH2 = 0.46,
T1 = 0.0275,
T2 = 0.0275,
Y = 0.85,
...
tableZHa = {AZ1sel, AZ2sel, AZ1, AZ2, H1pH2, T1, T2, X1p, X2p, Y, Z};
];
...
tableZHap3 = {AZ1sel, AZ2sel, AZ1, AZ2, H1pH2, T1*dec, T2*dec, X1p, X2p, Y*dec, Z*dec};
];
...
tableZHap2 = {AZ1sel, AZ2sel, AZ1, AZ2, H1pH2, T1*dec, T2*dec, X1p, X2p, Y*dec, Z*dec};
];
...
With[{T2p=T1p},
tableZHaB1={AZ1selp, AZ2selp, AZ1p, AZ2p, H1pH2p, T1p, T2p, X1p, X2p, Yp, Zp};
];];
...
With[{
  ...
  With[{T2p = T1p},
    tableZHaB5 = {AZ1selp, AZ2selp, AZ1p, AZ2p, H1pH2p, T1p, T2p, X1p,
       X2p, Yp, Zp};
    ];];


(* taken from table sent by Dave Aveline on 17 June 2021, saved as
210616_ZHaC_TransfertoWIndow_SM3.xlsx *)
...
With[{
  ...
  With[{T2p = T1p},
    tableZHaC = {AZ1selp, AZ2selp, AZ1p, AZ2p, H1pH2p, T1p, T2p, X1p,
       X2p, Yp, Zp};
    ];];


(* chosen for its centeredness and evenness, generated by exploring near the Cass trap (ZHaC) *)
...
With[{
  ...
  With[{T2p = T1p},
    tableZHaB6 = {AZ1selp, AZ2selp, AZ1p, AZ2p, H1pH2p, T1p, T2p, X1p,
       X2p, Yp, Zp};
    ];];
